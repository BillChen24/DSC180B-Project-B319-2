...
class IWildCamDataset_custom(WILDSDataset):
    """
    ...
    def __init__(self, version=None, root_dir='data', download=False, split_scheme='official'):
        ...
        # Load splits
        #df = pd.read_csv(self._data_dir / 'metadata_modified.csv')
        df = pd.read_csv('data/' + 'metadata1.csv')

        # Splits
        self._split_dict = {'train': 0, 'val': 1, 'test': 2, 'id_val': 3, 'id_test': 4,
                       'train_ignore': 5, 'val_ignore': 6,'id_val_ignore': 7, 'id_test_ignore': 8, 'test_ignore':9}
        self._split_names = {'train': 'Train', 'val': 'Validation (OOD/Trans)',
                             'test': 'Test (OOD/Trans)', 
                             'id_val': 'Validation (ID/Cis)',
                             'id_test': 'Test (ID/Cis)',
                             'train_ignore': 'Train no', 
                             'val_ignore': 'Validation (OOD/Trans)no',
                             'id_val_ignore': 'Validation (ID/Cis) no',
                             'id_test_ignore': 'Test (ID/Cis) no',
                             'test_ignore': 'Test (OOD/Trans) no'}
